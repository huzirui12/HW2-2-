...
class Model(Diffable):
    ...
    def evaluate(self, x, y, batch_size):
        ...
        for b, b1 in enumerate(range(batch_size, x.shape[0] + 1, batch_size)):
            b0 = b1 - batch_size
            batch_metrics = self.batch_step(x[b0:b1], y[b0:b1], training=False)  # Note the training=False
            update_metric_dict(agg_metrics, batch_metrics)
            print_stats(batch_metrics, b, batch_num)
        print_stats(agg_metrics, avg=True)  # Assuming print_stats can handle overall metrics without an epoch number
        return agg_metrics
